...
@Component({
  selector: 'app-user-magagement',
  templateUrl: './user-magagement.component.html',
  styleUrls: ['./user-magagement.component.scss']
})
export class UserMagagementComponent implements OnInit {
  imageSrc = '';
  showColor = false;
  showInfo = false;
  caption = 'Active';
...
if(status== 0) {this.changeStatusVal =1;}
  ...
  userDetails() {
    this.adminservice.HttpGetReq(`admin/userlist?token=${this.logtoken}`, true)
    .then(
      (response: any) => {
       this.userList = response.data;
       this.spinnerService.hide();
  ...
  showMyorder(userid: any){
    this.userid = userid;
    this.router.navigateByUrl('/main/orderlist/' + this.userid);
  }
