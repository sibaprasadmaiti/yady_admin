...
@Component({
  selector: 'app-cat-subcat-area-places',
  templateUrl: './cat-subcat-area-places.component.html',
  styleUrls: ['./cat-subcat-area-places.component.scss']
})
export class CatSubcatAreaPlacesComponent implements OnInit, AfterViewInit {
  ...
  constructor(private route: ActivatedRoute,
	...
	getAreaDetails(area_id) {
		...
		this.adminservice.HttpGetReq(`admin/getAreaDetails?token=${this.logtoken}&area_id=${area_id}`, true)
		.then((response: any) => {
			...
			if(response.data.sub_category_id == '' || response.data.sub_category_id == null) {
				this.subcat_id = 'none';
			} else {
				this.subcat_id = response.data.sub_category_id;
			}
	...
	deletePlace(area_id) {
		...
		if(area_id) {
			...
			this.adminservice.HttpPostReq(`admin/deleteAreaPlace?token=${this.logtoken}`, placeObj, true).then((response:any)=>{
