...
export class RecipelistComponent implements OnInit {
  ...
  constructor(public adminservice: ApiServiceService,
              ...
              public matDialog: MatDialog) { 
                ...
                this.defaultimg = environment.api_url + 'public/recipe/no_image.png';
              }
  ...
  addNew() {
    console.log('hi....');
    this.router.navigateByUrl('/main/addrecipe');
  }
  recipelistShow() {
    this.spinnerService.show();
    this.adminservice.HttpGetReq(`admin/recipelist?token=${this.logtoken}`, true)
    .then(
      (response: any) => {
  ...
  public changeStatus(index: any, id: any, status: any) {
    ...
    this.adminservice.HttpPostReq (`admin/changeRecipeStatus?token=${this.logtoken}`, formdata , true)
      .then(
        (response: any) => {

         // this.userList = response.data;
         this.recipeData[index].status = this.changeStatusVal;
         this.spinnerService.hide();
         this.toastr.success(response.message);
        },
       (error) => {
  ...
  editMenu(id: any){
    console.log('<<<id', id);
    this.router.navigateByUrl(`/main/editrecipe/${id}`);
  }
